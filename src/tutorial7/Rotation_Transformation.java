...
public class Rotation_Transformation implements GLEventListener {
    ...
    public static void main(String[] args) {
        ...
        canvas.addGLEventListener(new Rotation_Transformation());
...
//        FloatBuffer World = FloatBuffer.allocate(9);
        float[] World = {(float)Math.cos(Scale), -(float)Math.sin(Scale), 0f, 0,
            (float)Math.sin(Scale), (float)Math.cos(Scale), 0f, 0f,
            0f, 0f, 1f, 0f,
            0f, 0f, 0f, 1f};
    ...
    private String[] readShader(String path) {
        ...
        try {
            ...
            Logger.getLogger(Rotation_Transformation.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                is.close();
            } catch (IOException ex) {

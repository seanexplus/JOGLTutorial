...
public class Spot_Light implements GLEventListener {

    public Spot_Light(Camera camera) {
        this.camera = camera;
        camera.light = light;
    ...
    public static void main(String[] args) {
        ...
        canvas.addGLEventListener(new Spot_Light(camera));
...
//        gl.glUniform1i(gSampler, 0);
        ...
        try {
            ...
            Logger.getLogger(Spot_Light.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void render(GLAutoDrawable drawable) {
        ...
        sl[0].color = new float[] {0f,1f,1f};
        sl[0].position = new float[] {(float)camera.pos.get(0),(float)camera.pos.get(1),(float)camera.pos.get(2)};
        sl[0].direction =  new float[] {(float)camera.target.get(0),(float)camera.target.get(1),(float)camera.target.get(2)};
        sl[0].attenuation.linear = 0.1f;
        sl[0].cutoff = 10f;
        
        sl[1].diffuseIntensity = 0.75f;
        sl[1].color = new float[] {0f,0.5f,1f};
        sl[1].position = new float[] { 7f,1f, FieldDepth * ((float)Math.sin(Scale) + 1f) /2f};
        sl[1].attenuation.linear = 0.1f;
        
        effect.setSpotLights(sl);

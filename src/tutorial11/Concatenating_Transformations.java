...
public class Concatenating_Transformations implements GLEventListener {
    ...
    public static void main(String[] args) {
        ...
        JFrame frame = new JFrame("Concatenating Transformations");
        ...
        canvas.addGLEventListener(new Concatenating_Transformations());
...
//        float[] World = {(float)Math.cos(Scale), 0f, -(float)Math.sin(Scale), 0,
//            0f, 1, 0f, 0f,
//            (float)Math.sin(Scale), 0f, (float)Math.cos(Scale), 0f,
//            0f, 0f, 0f, 1f};
        
        long t1 = System.nanoTime();
        Pipeline p = new Pipeline();
        float tScale = (float)Math.sin(Scale*0.1f);
        float tTrans = (float)Math.sin(Scale);
        float fDegree = (float)Math.sin(Scale)*90;
        p.scale(tScale, tScale, tScale);
        p.transfrom(tTrans, 0, 0);
        p.rotate(fDegree, fDegree, fDegree);
...
//        gl.glDrawArrays(GL2.GL_TRIANGLES, 0, 3);
    ...
    private String[] readShader(String path) {
        ...
        try {
            ...
            Logger.getLogger(Concatenating_Transformations.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                is.close();
            } catch (IOException ex) {
